...
export default function AdsDepositRecordPage() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await fetch('/api/tiktok/deposit-records', {
        ...
        <table>
          ...
          <tbody>
            {loading ? (
              ...
              records.map((item) => (
                <tr key={item.id}>
                  <td className="greenText">TTREC{item.id.toString().padStart(8, '0')}</td>
                  <td>{item.account_name || '-'}</td>
                  <td>TT{item.account_id?.toString().padStart(8, '0') || '-'}</td>
